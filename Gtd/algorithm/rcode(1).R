...
#####2019/11/6
library(openxlsx)
data2 = read.xlsx("D:\\R\\Rfile\\_paper\\genes.xlsx",sheet = 1,colNames = T)
str(data2)

##corr
data = as.matrix(data2[1:8867,2:8])
data = t(data)
p = ncol(data)
cormatrix = matrix(0,p,p)
for (i in 1:(p-1)){
...
cormatrix = as.data.frame(cormatrix)
colnames(cormatrix) = data2[1:8867,1]
rownames(cormatrix) = data1[1:8867,1]
write.csv(cormatrix,"D:\\R\\Rfile\\_paper\\cormatrix2.csv",row.names = TRUE)
